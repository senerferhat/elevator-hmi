...
| **Phase 1 lab (2026-06-02)** | **LMT101 on MIPI LCD:** Linux stack **PASS** (jadard init, DSI **468×4**, DRM plane **96** @ 60 Hz); **backlit black** on glass (**BLK-014**). **BLK-006** closed (XRES **GPIO0_C6**). Vendor mail: `**docs/VENDOR-SUPPORT-LMT101-BRINGUP-EMAIL.txt`**. **TASK-106** software lab closed — gate = vendor FAE + MIPI scope. |
| **Phase 1 display (2026-08-07)** | ✅ **PANEL WORKS — BLK-014 RESOLVED. Display gate PASSED.** Full-screen white fill of `/dev/fb0` displayed **white on glass**; half fill → clean white/black split (correct addressing). VP1 ACTIVE, DSI-1, 800x1280p60, `real_clk[70000]`, vendor-exact porches. **Root cause = DSI init ordering** (this kernel enters VIDEO mode before `drm_panel_enable()`, so init was sent into a live video stream) — **fixed by patch 0021 INIT-IN-PREPARE**; stayed invisible because `CONFIG_FRAMEBUFFER_CONSOLE` was unset, so nothing ever rendered into `/dev/fb0` (zero-filled buffer = correctly-displayed black). fbcon + `CONFIG_LOGO` enabled 2026-08-07. **Never a hardware fault** — panels, 3.3 V rail, XRES, MIPI lanes, FPC all exonerated. Vendor correction owed. Day-to-day record: `diary/SESSION-LOG.md`. |
| **Phase 2 Qt image (2026-08-18)** | ✅ **HMI RENDERS ON GLASS** via **`linuxfb` + software**, not EGLFS/Mali (BLK-015). Four layouts, wrap default **52 px** (`hmi wrap`). Stage 1 SD plumbing: VFAT TF → `/media/sdcard`, QML pane shows card/clip status. **No video frames until BLK-015.** Flash target: newest `diary/SESSION-LOG.md`. |
| **🔴 BLK-015 (2026-08-18)** | **KMS scanout never reaches the panel.** Qt/EGLFS *and* `modetest` with a plain dumb buffer are equally invisible: the vop2 driver logs the correct address every frame, vblank runs at 60 Hz, two planes on VP1 commit independently — and the hardware keeps scanning the buffer latched at boot. Only writes *into* that buffer (`/dev/fb0`) display. **Consequence: no Mali GPU, and GStreamer zero-copy VPU video (§1 Media) is impossible until fixed** — video needs DRM planes. Deviation from ADR-001 (EGLFS/Mali). Next step: run the same `modetest` on the vendor's own image to separate "we broke it" from a BSP defect. Details + 6 falsified hypotheses in `diary/BLOCKERS.md`. |
| **Fonts (2026-08-18)** | Earlier Qt images shipped **no font files at all** (only `libfontconfig1`/`libfreetype6`), so every QML `Text` rendered blank over a near-black background — visually identical to a dead panel. Fixed with `liberation-fonts` + `fontconfig-utils`. |
...
- R-04: Adaptive backlight strategy — **RE-OPENED 2026-08-22**. The backlight ships with the LCD from the panel vendor and is not controllable at our end, so the "adaptive dimming in Phase 3" mitigation does not exist. See §6 R-04.
...
| R-04 | LED backlight lifetime ~2.3yr at 24/7 full brightness. Target is 5yr.                                                                                                                                                                      | **RE-OPENED** | **⚠ MITIGATION VOID (2026-08-22).** Was ACCEPTED on the basis of "adaptive dimming in Phase 3". Owner confirms the backlight is **supplied with the LCD from the panel vendor and is not controllable at our end**, so there is no dimming to schedule. Hardware corroborates: `pin gpio0-22 already requested by fe060000.dsi.0; cannot claim for fe6e0030.pwm` — GPIO0_C6 is the panel XRES, so the backlight PWM cannot claim its pin either. **A 2.3yr LED life against a 5yr field target now has no mitigation.** Needs a product decision: panel/backlight variant with dimming input, accept a mid-life backlight replacement in service, or re-negotiate the field-life target. |
