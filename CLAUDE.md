...
| **Phase 1 lab (2026-07-22)** | **Vendor BIST test: BLACK on BOTH panel units** (`bist-468.wic`, SHA `de0e0b60…`) — per vendor's own 7/7 criterion: **hardware issue confirmed**. Firmware audited zero-trust: init table 196/196 byte-identical, all timings vendor-exact. **VCC3V3_LCD now via stock load switch** (GPIO0_B7 active-high; Plan B jumper REMOVED; BLK-013 superseded — May "defective switch" was wrong-pin/polarity misdiagnosis). No further firmware permutations. **Waiting on vendor** — state dump `docs/VENDOR-SUPPORT-LMT101-BRINGUP-EMAIL-4.txt` (questions A–G). Day-to-day record: `diary/SESSION-LOG.md`. |
